...
Digital Identity makes loT objects unique and distinguishes them from each other. Self-Sovereign Identity (SSI) aims to provide a digital identity that is both verified and verifiable to loT objects while building a digital ecosystem for secure interactions between heterogeneous objects. However, in many real-world use cases, loT are constrained devices and cannot run natively a full SSI stack implementation. An edge device has been designed with the capability of supporting a constrained device to create and manage its own digital identity and it has been developed using Keystone, an open-source framework for building Trusted Execution Environments, allowing confidentiality and integrity of the offloaded operations. Such a solution has the advantage to increase the number of devices that can interact in such a secure digital ecosystem and defines a new paradigm called Self-Sovereign Identity as a Service. 
...
\input{chapters/chapter2-keystone.tex}
