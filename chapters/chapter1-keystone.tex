...
\begin{itemize}
    \item Programmability: unlike microcode for x86, in RISC-V M-mode software can be written using already existing toolchains and programming languages, such as C. 
    \item Agile Patching: since the TCB is purely software, bugs or vulnerabilities can be patched without the need for updates that are specific to particular hardware. 
    \item Verifiability: compared to hardware, the software is generally simpler to formally verify.
\end{itemize}
